...
const replaceText = (el, s)=>Array.from(el.childNodes).filter((node)=>node.nodeType === Node.TEXT_NODE).forEach((node)=>node.textContent = s);
const monthname = {
    "January": "01",
    "February": "02",
    "March": "03",
...
const isoMorphRaw = ({ xs, fn })=>xs.forEach((el)=>{
        fn(el);
        el.setAttribute("data-isomorph", "true");
    });
const isoMorph = ({ getTime, xs, fn = (el, time)=>void (el.innerText = toISODate(time)) })=>Array.from(xs).forEach((el)=>{
...
if (location.hostname.endsWith("github.com")) {
    const github = ()=>{
        ...
        isoMorph({
            xs: document.querySelectorAll(`div.TimelineItem-body > h3,h3[data-testid="commit-group-title"]`),
